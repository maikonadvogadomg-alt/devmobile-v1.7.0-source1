...
const PLUGINS: Plugin[] = [
  ...
  },
  {
    ...
    versions: [
      { label: "OpenJDK 21 LTS (mais recente)", value: "21", recommended: true, cmd: "command -v java >/dev/null 2>&1 && echo '✅ Java já instalado:' && java --version || (echo '⏳ Instalando Java 21 (pode demorar 3-5 min)...' && nix-env -iA nixpkgs.jdk21 2>&1 && export PATH=$HOME/.nix-profile/bin:$PATH && java --version && echo '✅ Java 21 instalado! Use: javac Arquivo.java && java Arquivo')" },
      { label: "OpenJDK 17 LTS", value: "17", cmd: "command -v java >/dev/null 2>&1 && java --version || (echo '⏳ Instalando Java 17...' && nix-env -iA nixpkgs.jdk17 2>&1 && export PATH=$HOME/.nix-profile/bin:$PATH && java --version && echo '✅ Java 17 instalado!')" },
    ],
  },
  {
    ...
    versions: [
      { label: "Kotlin via Nix", value: "2.0", recommended: true, cmd: "command -v kotlinc >/dev/null 2>&1 && echo '✅ Kotlin já instalado:' && kotlinc -version || (echo '⏳ Instalando Kotlin (pode demorar 5-10 min)...' && nix-env -iA nixpkgs.kotlin 2>&1 && export PATH=$HOME/.nix-profile/bin:$PATH && kotlinc -version && echo '✅ Kotlin instalado! Use: kotlinc Main.kt -include-runtime -d main.jar && java -jar main.jar')" },
    ],
  },
  {
  ...
  },
  {
    ...
    versions: [
      { label: "Go (mais recente)", value: "latest", recommended: true, cmd: "command -v go >/dev/null 2>&1 && echo '✅ Go já instalado:' && go version || (echo '⏳ Instalando Go (pode demorar 3-5 min)...' && nix-env -iA nixpkgs.go 2>&1 && export PATH=$HOME/.nix-profile/bin:$PATH && go version && echo '✅ Go instalado! Use: go run main.go')" },
    ],
  },
  {
...
export default function PluginsScreen() {
  ...
  const runInstall = async (p: Plugin, v: PluginVersion) => {
    addLine(`\n╔══════════════════════════════════════════╗`, "info");
    addLine(`║  Instalando ${p.name} ${v.label}`, "info");
    addLine(`╚══════════════════════════════════════════╝\n`, "info");
    addLine(`$ ${v.cmd}\n`, "info");

    if (!TERMINAL_API) {
      addLine("⚠️  Servidor não configurado. Acesse Configurações → Servidor Backend.", "err");
      addLine("\nPara instalar offline no celular, configure o Termux e insira a URL nas configurações.", "info");
      addLine(`$ ${v.cmd}`, "out");
      setInstallDone(true);
      setInstallOk(false);
      return;
    ...
    try {
      ...
      if (!res.ok) {
        addLine(`Erro HTTP ${res.status}: ${await res.text().catch(() => "")}`, "err");
        setInstallDone(true);
        setInstallOk(false);
        return;
      }
      ...
      if (Platform.OS === "web" || !res.body) {
        ...
        for (const line of text.split("\n")) {
          ...
          try {
            ...
            if (parsed.type === "stdout") addLine(parsed.data, "out");
            else if (parsed.type === "stderr") addLine(parsed.data, "err");
            else if (parsed.type === "exit") {
              success = parsed.data === "0";
            }
          } catch {}
        }
      } else {
      ...
      if (success) {
        ...
        addLine(`\n⚠️  Processo encerrado com erros. Verifique o log acima.`, "err");
        setInstallOk(false);
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        <View style={[styles.installOverlay, { backgroundColor: "#0a0a0a" }]}>
          ...
          {/* Status */}
          <View style={[styles.statusBar, { backgroundColor: installDone ? (installOk ? "#10b98122" : "#ef444422") : "#007acc22", borderBottomColor: installDone ? (installOk ? "#10b981" : "#ef4444") : "#007acc" }]}>
            <Text style={{ color: installDone ? (installOk ? "#10b981" : "#ef4444") : "#007acc", fontSize: 12, fontWeight: "700" }}>
              {!installDone ? "⏳ Instalando... aguarde" : installOk ? "✅ Instalação concluída com sucesso!" : "⚠️  Concluído com avisos"}
            </Text>
          </View>
          ...
          >
            {installLines.map((l, i) => (
              <Text
                ...
                style={[
                  styles.installLine,
                  l.type === "err" ? { color: "#f87171" } : l.type === "info" ? { color: "#00d4aa" } : { color: "#d4d4d4" },
                ]}
                selectable
              >
                {l.text}
