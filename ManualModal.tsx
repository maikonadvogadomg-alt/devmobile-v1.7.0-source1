...
export default function ManualModal({ visible, onClose }: ManualModalProps) {
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case "termux": return (
        <View>
          <Alert color="blue">{"📡 Termux = terminal Linux real no seu celular, sem precisar de internet ou servidor externo. Com isso o DevMobile funciona 100% offline."}</Alert>

          <H2>{"📲 Passo 1 — Instalar o Termux"}</H2>
          <P>{"Baixe o Termux pelo F-Droid (NÃO pela Play Store — a versão da Play Store está desatualizada e não funciona direito)."}</P>
          <Step n={1}>{"Abra o navegador e acesse: f-droid.org"}</Step>
          <Step n={2}>{"Busque por Termux e instale"}</Step>
          <Step n={3}>{"Permita instalar apps de fontes desconhecidas se aparecer a mensagem"}</Step>
          <Alert color="yellow">{"⚠️ Não instale pelo Google Play Store. Use somente o F-Droid."}</Alert>

          <H2>{"⚙️ Passo 2 — Configurar o Termux"}</H2>
          <P>{"Abra o Termux e execute os comandos abaixo um por um:"}</P>
          <H3>{"Atualizar pacotes:"}</H3>
          <Code copyKey="termux-update">{"pkg update && pkg upgrade -y"}</Code>
          <H3>{"Instalar Node.js e Git:"}</H3>
          <Code copyKey="termux-node">{"pkg install nodejs git -y"}</Code>
          <H3>{"Verificar se instalou certo:"}</H3>
          <Code copyKey="termux-check">{"node --version\nnpm --version\ngit --version"}</Code>
          <Alert color="green">{"✅ Se aparecer v22 ou v24 para o node e v10+ para npm, está tudo certo."}</Alert>

          <H2>{"📥 Passo 3 — Instalar o servidor DevMobile"}</H2>
          <P>{"O servidor é um pacote Node.js que roda no Termux. Instale diretamente via npm (conexão com internet necessária só neste passo):"}</P>
          <Code copyKey="termux-install-server">{"npm install -g @devmobile/server"}</Code>
          <Alert color="yellow">{"⏳ Se o pacote acima não funcionar, use a alternativa abaixo (instala direto do GitHub):"}</Alert>
          <Code copyKey="termux-clone-alt">{"npm install -g https://github.com/oab183/caldeira/tarball/main"}</Code>
          <Alert color="yellow">{"⏳ Pode demorar 3-5 minutos na primeira vez. Aguarde terminar."}</Alert>

          <H2>{"▶️ Passo 4 — Iniciar o servidor"}</H2>
          <P>{"Toda vez que quiser usar o DevMobile offline, abra o Termux e execute:"}</P>
          <Code copyKey="termux-start">{"devmobile-server"}</Code>
          <P>{"O servidor vai mostrar uma mensagem como:"}</P>
          <Code>{"🚀 API Server running on port 8080\n✅ Pronto para receber conexões"}</Code>
          <Alert color="green">{"✅ Enquanto essa mensagem aparecer, o servidor está rodando. Deixe o Termux aberto em segundo plano (minimize, não feche)."}</Alert>

          <H2>{"🔗 Passo 5 — Conectar o DevMobile ao Termux"}</H2>
          <P>{"Agora diga ao DevMobile para usar o servidor local do Termux em vez do Replit:"}</P>
          <Step n={1}>{"No DevMobile, vá em: Configurações (ícone de engrenagem)"}</Step>
          <Step n={2}>{"Role para baixo até a seção SERVIDOR BACKEND"}</Step>
          <Step n={3}>{"Toque em Padrão (Replit) para editar"}</Step>
          <Step n={4}>{"Digite: http://localhost:8080"}</Step>
          <Step n={5}>{"Toque em Salvar"}</Step>
          <Alert color="green">{"✅ Pronto! Agora o terminal, a IA Cortesia e os plugins vão usar o Termux. Funciona mesmo sem internet."}</Alert>

          <H2>{"🔄 Como usar no dia a dia"}</H2>
          <Li>{"1. Abra o Termux"}</Li>
          <Li>{"2. Digite: devmobile-server"}</Li>
          <Li>{"3. Minimize o Termux (não feche)"}</Li>
          <Li>{"4. Abra o DevMobile e use normalmente"}</Li>
          <Alert color="blue">{"💡 É só isso! Dois apps abertos ao mesmo tempo: Termux rodando o servidor + DevMobile usando ele."}</Alert>

          <H2>{"❓ Problemas comuns"}</H2>
          <H3>{"\"command not found: devmobile-server\""}</H3>
          <P>{"O servidor não foi instalado ainda. Volte ao Passo 3 e execute a instalação."}</P>
          <H3>{"\"Port 8080 already in use\""}</H3>
          <Code copyKey="fix-port">{"pkill -f devmobile-server\ndevmobile-server"}</Code>
          <H3>{"Terminal mostra \"Servidor não configurado\""}</H3>
          <Li>{"Verifique se o Termux está aberto e o servidor rodando"}</Li>
          <Li>{"Verifique se a URL nas configurações está exatamente: http://localhost:8080"}</Li>
          <H3>{"Quer voltar a usar o servidor Replit"}</H3>
          <Li>{"Vá em Configurações → Servidor Backend"}</Li>
          <Li>{"Toque em Limpar (voltar para Replit)"}</Li>
        </View>
      );
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose} statusBarTranslucent>
      <View style={{ flex: 1, backgroundColor: bg }}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingTop: insets.top + 6, paddingBottom: 10, backgroundColor: card, borderBottomWidth: 1, borderBottomColor: border, gap: 10 }}>
          <Text style={{ color: fg, fontWeight: "700", fontSize: 17, flex: 1 }}>📖 Manual do DevMobile</Text>
