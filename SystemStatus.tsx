...
export default function SystemStatus({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const { aiProviders, dbConfigs, gitConfigs, activeProject, terminalSessions } = useApp();
  ...
  const runAllChecks = useCallback(async () => {
    ...
    if (!API_BASE) {
      ...
      try {
        ...
        update("api", { stat: r?.ok ? "ok" : "fail", detail: r?.ok ? `Rodando em ${API_BASE} ✅` : `HTTP ${r?.status} ❌` });
      } catch {
        update("api", { stat: "fail", detail: "Servidor não acessível ❌" });
      }
    }
    ...
    if (API_BASE) {
      try {
        ...
        update("ai", {
          stat: r?.ok ? "ok" : "warn",
          detail: r?.ok ? "Gemini respondendo ✅ (sem precisar de API key)" : `HTTP ${r?.status}`,
        });
      } catch {
        update("ai", { stat: "fail", detail: "Endpoint /api/ai/chat não acessível ❌" });
      }
    } else {
      update("ai", { stat: "warn", detail: "Servidor não configurado" });
    }
        ...
        <ScrollView ref={scrollRef} style={{ flex: 1 }} contentContainerStyle={{ padding: 12, paddingBottom: 20 }}>
          ...
          {done && (
            <View style={{ marginTop: 20, backgroundColor: "#0d1f0d", borderWidth: 1, borderColor: "#1f4d1f", borderRadius: 12, padding: 14 }}>
              ...
              {[
                "Conexão à internet — o terminal roda no servidor, não no celular",
                "Servidor API ativo — aparece como 'OK' no status acima",
                "Para APK offline completo: precisa do build via EAS (EXPO_TOKEN)",
              ].map((item, i) => (
                <View key={i} style={{ flexDirection: "row", gap: 8, marginBottom: 6 }}>
